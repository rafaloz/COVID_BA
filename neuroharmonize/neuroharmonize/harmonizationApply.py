...
def harmonizationApply(data, covars, model, ref_batch=None, return_stand_mean=False):
    """
    ...
    if sum(isTrainSite)==0:
        ...
        stand_mean = np.asarray(stand_mean)
        mod_mean   = np.asarray(mod_mean)
        var_pooled = np.asarray(var_pooled)
        data       = np.asarray(data)
        isTrainSite= np.asarray(isTrainSite, dtype=bool)
        bayes_data = adjust_data_final(s_data, design, model['gamma_star'], model['delta_star'], stand_mean, mod_mean, var_pooled, info_dict, data)
