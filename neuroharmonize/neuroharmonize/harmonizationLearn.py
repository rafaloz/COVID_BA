...
def harmonizationLearn(data, covars, eb=True, smooth_terms=[],
    ...
    ref_batch (Optional): batch (site or scanner) to be used as reference for batch adjustment.
        - None by default
    ...
    if smooth_model['perform_smoothing']:
        ...
        if orig_model is None:
            bs = BSplines(X_spline, df=[10] * len(smooth_cols), degree=[3] * len(smooth_cols),
                        knot_kwds=[{'lower_bound':smooth_term_bounds[0], 'upper_bound':smooth_term_bounds[1]}]*len(smooth_cols))
    ...
    if orig_model is None:
        s_data, stand_mean, var_pooled, B_hat, mod_mean, grand_mean = standardizeAcrossFeatures(data, design, info_dict, smooth_model)
        LS_dict = fitLSModelAndFindPriors(s_data, design, info_dict, eb=eb)
        # optional: avoid EB estimates
        if eb:
            gamma_star, delta_star = find_parametric_adjustments(s_data, LS_dict, info_dict, False)
        else:
        ...
        if batch_levels.size == 0:
            ...
            s_data_train, stand_mean_train, v_pool, mod_mean = applyStandardizationAcrossFeatures(data[:, isTrainSite], tmp[isTrainSite, :], info_dict_train, model)
            design2 = tmp.copy()
            design2[:, isTrainSiteColumnsOrig[0]] = design[:,isTrainSiteColumns[0]]
            bayes_data_train = adjust_data_final(s_data_train, design2[isTrainSite,:], model['gamma_star'], model['delta_star'], model['mod_mean'], model['var_pooled'], info_dict_train, data[:, isTrainSite])
            # transpose data to return to original shape
        ...
        if batch_levels.size == 0:
            ...
            s_data_test, stand_mean_test, v_pool, mod_mean = applyStandardizationAcrossFeatures(data[:,~isTrainSite], design_tmp[~isTrainSite,:], info_dict_test, model)
            ...
            bayes_data_test = adjust_data_final(s_data_test, design_tmp[~isTrainSite,:], gamma_star, delta_star, stand_mean_test, model['mod_mean'], model['var_pooled'], info_dict_test, data[:,~isTrainSite])
            # transpose data to return to original shape
...
def standardizeAcrossFeatures(X, design, info_dict, smooth_model):
    ...
    if smooth_model['perform_smoothing']:
        ...
        for i in range(0, X.shape[0]):
            ...
            print(f"[neuroHarmonize]: Processed feature {i + 1}/{X.shape[0]}, ({current_time})")
    ###
    else:
    ...
    return s_data, stand_mean, var_pooled, B_hat, mod_mean, grand_mean
...
def fitLSModelAndFindPriors(s_data, design, info_dict, eb=True):
    ...
    _combat_diagnostics_soft(gamma_hat, delta_hat, info_dict, alpha=0.001)
...
def saveHarmonizationModel(model, file_name):
    ...
    est_size = 0
    for key in ['design', 'B_hat', 'mod_mean', 'var_pooled',
...
def _combat_diagnostics(gamma_hat, delta_hat, info_dict, alpha=0.001):
    """
    One KS / D’Agostino–Pearson test per batch.
    Warn if the Normal or Inv-Gamma assumptions look implausible.
    """
    n_batch = info_dict['n_batch']
    problems = []
    ...
    for b in range(n_batch):
        ...
        # --- Normality of gamma_hat -----------------------------------------
        if g.size >= 10:                 # normaltest needs n ≥ 20
            stat_g, p_g = normaltest(g, nan_policy="omit")
            if p_g < alpha:
                problems.append(f"Batch {b}: γ̂ not normal (p={p_g:.2g})")

        # --- Inv-Gamma fit for delta_hat ------------------------------------
        # fit inverse-gamma MLE
        a_mle, loc_mle, scale_mle = invgamma.fit(d, floc=0)  # loc fixed to 0
        stat_d, p_d = kstest(d, "invgamma", args=(a_mle, loc_mle, scale_mle))
        if p_d < alpha:
            problems.append(f"Batch {b}: δ̂ not Inv-Gamma (p={p_d:.2g})")

    if problems:
        warnings.warn(
            "[ComBat-Diag] "
            + "; ".join(problems)
            + ". Consider non-parametric ComBat for those batches.",
            stacklevel=2,
        )



def _combat_diagnostics_soft(
        gamma_hat,
        delta_hat,
        info_dict,
        alpha=0.001,
    ...
    for b in range(n_batch):
        g = np.asarray(gamma_hat[b, :]).ravel()
        d = np.asarray(delta_hat[b]).ravel()

        # ---------- γ̂ : normalidad -----------------------------------------
        w,   p_w   = shapiro(g) if g.size >= 3 else (np.nan, np.nan)
        g_skew     = skew(g, bias=False, nan_policy="omit")
        g_kurt_ex  = kurtosis(g, fisher=True, bias=False, nan_policy="omit")
        z_skew     = g_skew  / np.sqrt(6 / g.size)
        z_kurt     = g_kurt_ex / np.sqrt(24 / g.size)

        is_gamma_ok = (abs(g_skew) < 2) and (abs(g_kurt_ex) < 7)

        # ---------- δ̂ : ajuste Inv-Gamma (con piso suave) ----------
        a_mle, loc_mle, scale_mle = invgamma.fit(d, floc=0)
        a_mle = max(a_mle, 2.01)                          # asegura varianza finita

        # (opcional) test KS, lo conservamos por si quieres revisarlo
        D, p_ks = kstest(d, "invgamma", args=(a_mle, loc_mle, scale_mle))

        # --- momentos teóricos y empíricos ---
        cv_th  = 1 / np.sqrt(a_mle - 2)                 # teórico
        cv_emp = np.std(d, ddof=1) / np.mean(d)         # empírico
        ratio  = cv_emp / cv_th

        # -------- regla de decisión -------------
        # Acepta si la dispersión empírica está entre 0.1× y 5× la teórica
        is_a_ok = np.isfinite(ratio) and (2 <= a_mle <= 10)


        # ---------- logging -------------------------------------------------
        print(
            f"Batch {b:2d} | γ̂: W={w:6.3f} p={p_w:.1e} "
            f"|g_skew|={abs(g_skew):4.2f} |g_kurt|={abs(g_kurt_ex):4.2f} "
            f"→ {'OK' if is_gamma_ok else 'flag'}\n"
            f"          δ̂: D={D:5.3f} p={p_ks:.1e} "
            f"a={a_mle:5.3f} "
            f"→ {'OK' if is_a_ok else 'flag'}"
        )

        # ---------- collect problems ----------------------------------------
        if not is_gamma_ok:
            problems.append(f"Batch {b}: γ̂ deviates (|Z|>{z_thresh})")
        if not is_a_ok:
            problems.append(f"Batch {b}: a out of range")

    if problems:
        warnings.warn(
            "[ComBat-SoftDiag]\n  - "
            + "\n  - ".join(problems)
            + "\nRevisa estos batches o usa ComBat no paramétrico.",
            stacklevel=2,
        )
