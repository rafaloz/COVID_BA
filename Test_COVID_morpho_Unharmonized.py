...
def raincloud_plot(
    ...
    box_width=0.35, kde_alpha=0.4, point_size=30, mirror=True,
    ...
    for i, data in enumerate(data_list):
        ...
        # Median marker on center
        med = np.median(data)
        ax.plot([pos - width*0.9, pos + width*0.9], [med, med], linewidth=2, zorder=3, color=color)
...
for _, row in wide.iterrows():
    plt.plot([row['Edad_t1'], row['Edad_t2']],
             [row['pred_Edad_c_t1'], row['pred_Edad_c_t2']],
             linewidth=0.8)

# Puntos T1 y T2 (marcadores distintos)
plt.scatter(wide['Edad_t1'], wide['pred_Edad_c_t1'], s=90, label='T1 (baseline)', color='#A74A4A')
plt.scatter(wide['Edad_t2'], wide['pred_Edad_c_t2'], s=90, label='T2 (follow-up)', color='#9D9D4F')

# Línea identidad y ejes
xmin = np.nanmin([wide['Edad_t1'].min(), wide['Edad_t2'].min()])
xmax = np.nanmax([wide['Edad_t1'].max(), wide['Edad_t2'].max()])
ymin = np.nanmin([wide['pred_Edad_c_t1'].min(), wide['pred_Edad_c_t2'].min()])
ymax = np.nanmax([wide['pred_Edad_c_t1'].max(), wide['pred_Edad_c_t2'].max()])
...
bp1 = pairs['BrainPAD_c_t1'].to_numpy()
bp2 = pairs['BrainPAD_c_t2'].to_numpy()
delta = bp2 - bp1
n = delta.size
df = n - 1
