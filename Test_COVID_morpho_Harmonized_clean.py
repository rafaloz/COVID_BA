...
def update_prededad(results_df, predictions_df, id_col_results='ID', id_col_pred='id', pred_col='prediction'):
    r = results_df.copy()
...
def plot_bag_age_side_by_side(df: pd.DataFrame, age_col: str = "Edad", bag_before_col: str = "BAG",  bag_after_col: str = "BAG_corr",
    figsize=(10, 5), dpi=300, outfile: str | None = None, annotate: bool = True, symmetric_ylim: bool = True):
    """
...
def raincloud_plot(data_list, labels, width=0.35, jitter=0.06, bandwidth='scott',
...
def plot_pred_vs_age_two_groups(df_controls, df_patients,  x_col='Edad', y_corr_col='pred_Edad', y_raw_col='pred_Edad', labels=('Controls', 'Patients'), markers=('o', 'o'), outfile_base=None):
...
X_test_results = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_HoldOutTest_results.csv'))
X_test = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_HoldOutTest.csv')).values
...
print('######## Result hold out test ##########')
res = summarize_metrics(result_x_test, y_col="Edad", yhat_col="pred_Edad", sex_col="sexo(M=1;F=0)", B=5000, seed=42)
print(res)

result_AgeRisk = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_OutOfSample_AgeRisk_results.csv'))
AgeRisk = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_OutOfSample_AgeRisk.csv')).values

result_AgeRisk = model_evaluation_clean(AgeRisk, result_AgeRisk)
figura_edad_y_edad_predicha(result_AgeRisk['Edad'].values, result_AgeRisk['pred_Edad_c'].values)

print('######## Result out of sample ##########')
...
controles_COVID = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_Controls.csv')).values
...
Harmonized_COVID = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_COVID.csv')).values

pac_COVID_results = model_evaluation_clean(Harmonized_COVID, Harmonized_COVID_results)
figura_edad_y_edad_predicha(pac_COVID_results['Edad'], pac_COVID_results['pred_Edad'])

print('######## Result Patients COVID ##########')
...
pac_COVID_II = pd.read_csv(os.path.join(carpeta_datos, 'harmonized', 'Harmonized_Longitdunal.csv')).values
...
print(pac_COVID_results_30.shape)

print('######## Result Patients COVID t1 ##########')
res = summarize_metrics(pac_COVID_results_30, y_col="Edad", yhat_col="pred_Edad", sex_col="sexo(M=1;F=0)", B=5000, seed=42)
print(res.to_string(index=False, max_rows=None, max_cols=None))

print('######## Result Patients COVID t2 ##########')
...
print('\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
print('^^^^^^^^^^^^^^^^ ARE THE GROUPS COMPARABLE IN AGE AND SEX ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n')
...
# Build 2×2 table: rows = sex (M, F), cols = groups (Healthy, COVID)
