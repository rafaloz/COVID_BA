...
def _twosided_power_from_nct(t_crit, df, ncp):
    # power = P(|T| > t_crit) con T ~ nct(df, ncp)
...
def estimate_sd_and_r2_from_df(df, y='BAG_raw', covars=('Age','Sex','eTIV','Euler_z')):
    """
...
# df_base: una fila por sujeto en basal (controles + COV), columnas:
# ['BAG_raw','Age','Sex','eTIV','Euler_z','Group', ...]
sd_y, r2 = estimate_sd_and_r2_from_df(PA_Baseline, y='BrainPAD', covars=('Age','sex','eTIV','euler_number_z'))
delta = 3.0
alpha = 0.05
p_cov = 4

# Potencia con N actuales (ejemplo n1=80, n2=87)
n1, n2 = 48, 53
...
dif = wide['BrainPAD_c_t2'] - wide['BrainPAD_c_t1']
n  = len(dif)
